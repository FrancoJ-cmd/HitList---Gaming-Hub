...
class LocalDataSourceImplTest {
    ...
    @Test
    fun `given saved deals, they are retrieved by game name`() {
        val deals = listOf(Deal("Steam", "0.00", "29.99", 100.0, "0.00"))
        dataSource.saveDeals("Dota 2", deals)
        val loaded = dataSource.getDeals("Dota 2")
        assertNotNull(loaded)
        assertEquals(deals, loaded.first)
    }
}
